...
class Renderer {
  ...
  setUniformData(model, camera, shaderData) {
    ...
    let worldMatrix = camera.cameraMatrix;
    ...
    gl.uniformMatrix3fv(
      invtransLoc,
      false,
      M4.inverseTranspose3x3(model.modelMatrix).toFloat32()
    );

    let lightDirLoc = gl.getUniformLocation(this.program, "u_directionalLight");
    gl.uniform3fv(
      lightDirLoc,
      shaderData.lightingData.directionalLight.toFloat32()
    );

    let lightColLoc = gl.getUniformLocation(this.program, "u_directionalColor");
    gl.uniform3fv(
      lightColLoc,
      shaderData.lightingData.directionalColor.toFloat32()
    );
    ...
    var worldLocation = gl.getUniformLocation(this.program, "u_world");
    gl.uniformMatrix4fv(worldLocation, false, worldMatrix);

    gl.uniform3fv(lightWorldPositionLocation, [30, 40, 60]);
  }
}
